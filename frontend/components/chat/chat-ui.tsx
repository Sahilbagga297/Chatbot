...
export default function ChatUI() {
    ...
    try {
      ...
      if (isHoneypotMode || isJson) {
        ...
        if (ttsEnabled && assistantMsg.content) {
          speak(assistantMsg.content, () => setSpeaking(true), () => setSpeaking(false))
        }
      } else {
  ...
  function replayLastReply() {
    ...
    stopSpeaking()
    speak(lastAssistant.content, () => setSpeaking(true), () => setSpeaking(false))
  }
          ...
          <div className="bg-slate-800/80 p-4 border-b border-slate-700 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <Button 
                ...
                className={`h-8 text-[10px] font-bold border-2 tracking-widest transition-all rounded-none ${
                  isHoneypotMode 
